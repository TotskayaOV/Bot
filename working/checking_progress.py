...
def checking_time(kicks_tuples: list):
    """
    Создает переменную в которую передается время в данный момент. Проходится циклом по все элементам списка,
    в котором сравнивается разница времени в кортеже и текущим временем с периодами в 5-6 минут и в 10 минут.
    Если данные соответствуют условия больше 5 минут, меньше 6 минут, отправляется сообщение через функцию call_coor
    всем координаторам. Если данные соответствуют 10 минутам, отправляется сообщение координаторам, задача удаляется
    из таблицы слежения за задачами.
    :param kicks_tuples: лист с кортежами из базы данных.
            в каждом кортеже(id задачи, время последнй обработки, количество обработок)
    """
